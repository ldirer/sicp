;source: https://github.com/l0stman/sicp/blob/master/4.67.query.scm
;(require r5rs/init)
          ...
          (else
           ...
           (display-stream
            (stream-map
             ...
             (qeval q (singleton-stream '()) empty-history)))
           (query-driver-loop)))))
...
(define (free-var? var frame)
  (let ((val (binding-in-frame var frame)))
    (or (not val)
        (and (var? val)
             (free-var? val frame)))))
...
(define (make-history query-pattern frame)
  (cons query-pattern (freevars query-pattern '())))
